...
## Browse TFregulomeR data warehouse

TFregulomeR allows users to easily browse the a large compendium of TFBSs using only one simple
function `TFBSBrowser`. Users can search the TFBS according to species, organ, sample type,
...
no input is given for the function, all records in TFregulomeR warehouse will be returned.

```{r eval=FALSE}
library(TFregulomeR)
# browse all records in TFregulomeR TFBS warehouse
...
#>       cell_tissue_name                      description disease_state     TF source
#> 1 embryonic-stem-cells             embryonic stem cells        normal PRDM14   GTRD
#> 2       CD4pos-T-cells                     CD4+ T-cells        normal    YY1   GTRD
#> 3               EWS502                    Ewing sarcoma         tumor   FLI1   GTRD
#> 4                HUVEC umbilical vein endothelial cells        normal   FLI1   GTRD
#> 5                LS180                     colon cancer         tumor TCF7L2   GTRD
#> 6                LS180                     colon cancer         tumor  CEBPB   GTRD
#>   source_ID peak_num peak_with_motif_num
#> 1 EXP000061    17482                5656
#> 2 EXP000080    18298                2038
#> 3 EXP000128    89523               28914
#> 4 EXP000132    71437               36126
#> 5 EXP000140    16517                2312
#> 6 EXP000142   128542               67747
...
We have designed some useful functions for data retrival from TFregulomeR compendium. 
...
(all peaks or peaks with motif only) from TFregulomeR warehouse using `loadPeaks`.
...
of all TFs in TFregulomeR warehouse using `loadPeaks`. To be noted, the peak regions of a given 
TF in TFregulomeR warehouse are the peak summits (hg38 for human), and the TFBS is enriched 
...
During the analyisis, EACH of target peak set from TFregulomeR compendium using `target_peak_id` and/or user provided using 
...
for each target common peak sub-ensemble, if its ID labeled in `target_peak_id` and `user_target_peak_list` matches a MethMotif ID ("MM1_HSA_").
...
# share with the CEBPB peaks in all cell types from TFregulomeR warehouse,
...
#> not orginated from TFregulomeR or the number of direct binding sites in the 
...
# customised input peaks are not orginated from TFregulomeR compendium, so no DNA methylation states
...
`intersectPeakMatrix` perform a exhaustive pair-wise intersection analysis between peak list x and peak list y, 
...
`intersectPeakMatrixResult`, user can obtain a intersection matrix table.  Row i 
...
TFregulomeR also allows uers to plot the distributions of a given TFBS from the TFregulomeR warehouse in a list of peak sets, using `motifDistrib` and `plotDistrib` sequentially. By providing the TFregulomeR ID in `id` as the input of the `motifDistrib`, `motifDistrib` will calculate the occurrences the TFBSs in the given list of peak sets input in `peak_list`. The unique IDs correpsonding to `peak_list` is required to be provided at the same time using `peak_id`. If a peak set is derived from TFregulomeR compendium, the TFregulomeR ID should be provided correspondingly; if it is self provided, you can name it with a unique ID yourself. 
